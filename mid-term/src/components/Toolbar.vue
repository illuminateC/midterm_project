<template>
    <div>
        <div class="toolbar">
            ...
            <el-button @click="passPermission">生成预览</el-button>
            <el-button @click="stopPermission">关闭预览</el-button>
            <el-button @click="exportHtml">导出HTML、JavaScript、CSS文件</el-button>

            <div class="canvas-config">
                <span>画布大小</span>
                <input v-model="canvasStyleData.width" @input="handleWidthChange">
                <span>*</span>
                <input v-model="canvasStyleData.height" @input="handleHeightChange">
            </div>
            <div class="canvas-config">
                <span>画布比例</span>
...
export default {
    ...
    methods: {
        ...
        handleWidthChange() {
            this.$emit('width-change', this.canvasStyleData.width);
        },
        handleHeightChange() {
            this.$emit('height-change', this.canvasStyleData.height);
        },
        handleScaleChange() {
        ...
        save() {
            ...
            formData.append('prototypeID', this.$route.params.prototypeID);
            axios.post('http://43.143.140.26/api/prototype/saveprototype', formData, {
                headers: { "content-type": "application/x-www-form-urlencoded" },
            })
                .then(res => {              /* res 是 response 的缩写 */
                    switch (res.data.errno) {
                        case 0: {
                            this.$message.success("保存原型图成功！");
                            break;
                        }
                        default: {
                            this.$message.warning("保存原型图失败！");
                            break;
                        }
                    }
                })
        ...
        passPermission() {
            ...
            axios.post('http://43.143.140.26/api/prototype/createpreview', formData, {
                headers: { "content-type": "application/x-www-form-urlencoded" },
            })
                .then(res => {              /* res 是 response 的缩写 */
                    switch (res.data.errno) {
                        case 0: {
                            this.$message.success("预览链接已复制到粘贴板!");
                            const previewUrl = window.location.protocol + '//' + window.location.hostname + (window.location.port ? (':' + window.location.port) : '') + '/visit/' + this.$route.params.projectID;
                            this.copyToClipboard(previewUrl);
                            break;
                        }
                        default: {
                            this.$message.warning("预览链接打开失败！");
                            break;
                        }
                    }
                })
        ...
        stopPermission() {
            let formData = new FormData();
            formData.append('projectId', this.$route.params.projectID);
            axios.post('http://43.143.140.26/api/prototype/closepreview', formData, {
                headers: { "content-type": "application/x-www-form-urlencoded" },
            })
                .then(res => {              /* res 是 response 的缩写 */
                    switch (res.data.errno) {
                        case 0: {
                            this.$message.success("预览链接已关闭!");
                            break;
                        }
                        default: {
                            this.$message.warning("预览链接关闭失败！");
                            break;
                        }
                    }
                })
