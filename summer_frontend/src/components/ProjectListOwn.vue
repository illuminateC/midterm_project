<template>
  <div>
    ...
    <el-button round @click="collectSelectItem" style="float: left; border: none; margin-top: 10px;"
      v-if="selectType === 'selection'" size="medium">收藏</el-button>
    ...
    <div v-if="showType === 'table'" class="table-container">
      <el-table height="500" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" size="medium"
        @filter-change="handleFilterChange" @cell-mouse-enter="handleRowMouseEnter"
        ...
        <el-table-column prop="finished" label="项目状态" width="180"
      ...
      <div style="width: 100%; flex-wrap: wrap; display: flex;">
        <div class="docment-container" v-for="table in tableData" style="display: inline-block;">
          <div class="document" @mouseover="hover = true" @mouseleave="hover = false">
            <div class="layer layer111111" :class="{ hidden: !hover }">
              <div class="icon-row">
                <i class="el-icon-edit" @click="editDocument(table.project_id)"></i>
                <i class="el-icon-star-off" @click="collectDocument(table.project_id)"></i>
              </div>
              <div class="icon-row">
            ...
            <div class="layer layer222222"></div>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleFilterChange(filters) {
      ...
      const columnKey = 'el-table_2_column_7'; // Replace this with your actual columnKey
    ...
    collectSelectItem() {
      this.multipleSelection.forEach((value, index) => {
        ...
        formData.append('team_id', this.$route.params.teamID);
        formData.append('project_id', value.project_id);
        axios.post('http://43.143.140.26/api/project/collect', formData, {
          headers: { "content-type": "application/x-www-form-urlencoded" },
        })
          .then(res => {              /* res 是 response 的缩写 */
            switch (res.data.errno) {
              ...
              case 1002: {
                this.$message.warning("您已收藏该项目");
                break;
              }
              default: {
                this.$message.warning("收藏项目失败！");
                break;
              }
            }
          })
...
.document:hover .layer111111 {
  visibility: visible;
}

.document:hover .layer222222 {
  opacity: 0.3;
}
