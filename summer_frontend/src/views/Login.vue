...
<template>
  <div id="login">
    <video autoplay loop muted id="bgvid">
      <source src="../assets/background/nightBackground.mp4" type="video/mp4">
    </video>
...
export default {
  ...
  methods: {
    ...
    onSignIn: function () {
      ...
      //   url: 'http://43.143.140.26/api/user/login',       /* 指明后端 api 路径，由于在 main.js 已指定根路径，因此在此处只需写相对路由 */
      //   headers: { "content-type": "application/x-www-form-urlencoded" },
      ...
      axios.post('http://43.143.140.26/api/user/login', formData, {
        headers: { "content-type": "application/x-www-form-urlencoded" },
      })
        .then(res => {              /* res 是 response 的缩写 */
          switch (res.data.errno) {
            case 0:
              ...
              sessionStorage.setItem("username", this.signinUsername);
              const history_pth = sessionStorage.getItem('preRoute');
              /* 若保存的路由为空或为注册路由，则跳转首页；否则跳转前路由（setTimeout表示1000ms后执行） */
              this.$message.success("登录成功！");
              setTimeout(() => {
                this.$router.push('/homepage/3');
                // if (history_pth == null || history_pth == '/homepage') {
                //   console.log("history" + history_pth);
                //   this.$router.push('/');
                // } else {
                //   console.log("history" + history_pth);
    ...
    onSignUp: function () {
      if (this.signupUsername === '' || this.signupPassword === '' || this.signupConfirmPassword === '' || this.email === '' || this.realname === '') {
      ...
      if (this.email === '') {
        this.$message.warning("请输入您的emial！");
        return;
      }
      ...
      formData.append('password_1', this.signupPassword);
      formData.append('password_2', this.signupPassword);
      ...
      formData.append('email', this.email);
      axios.post('http://43.143.140.26/api/user/register', formData, {
        headers: { "content-type": "application/x-www-form-urlencoded" },
      })
        .then(res => {
          switch (res.data.errno) {
